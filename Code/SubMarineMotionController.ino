/*
  Project: IMU-Based Orientation Tracking and Motor Control
  Author : Lakshan Lavan
  Email  : lakshanlavan29@gmail.com
  Date   : June 2024

  Description:
  This Arduino-based system reads real-time motion data from an MPU-6050
  accelerometer + gyroscope sensor, applies filtering, and outputs orientation
  values. Data is used for visualization (e.g., in LabVIEW) and controls
  motor speed/direction via Modbus RTU protocol.

  Requirements:
  - Arduino Uno (or compatible)
  - MPU-6050 IMU sensor (I2C)
  - L298N Motor Driver
  - LabVIEW (for UI) or Modbus Master
  - Arduino IDE 1.5.0 or later (for Wire.endTransmission(bool))

  References:
  - InvenSense MPU-6000/MPU-6050 Register Map
  - AN-MPU-6000EVB.pdf - Evaluation Board Guide

  Notes:
  - The IMU provides 16-bit accuracy for acceleration and gyro data
  - Temperature sensor range: -40 to +85°C, scale: 340 units/°C
  - At power-up:
      - Register 0x6B = 0x40 (PWR_MGMT_2)
      - Register 0x75 = 0x68 (WHO_AM_I)
*/
...
// Motor control pins (replace with actual pin numbers)
#define motorSpeedPin 9   // PWM pin for motor speed control
#define motorDirectionPin1 10  // Digital pin for motor direction control
#define motorDirectionPin2 11  // Digital pin for motor direction control
...
// Replace with the actual motor control class from the library
L298N motorController(motorSpeedPin, motorDirectionPin1, motorDirectionPin2);
...
regBank.setId(1);
  ...
  // Add registers for motor speed and direction
  regBank.add(40001);  // Motor speed register
  regBank.add(40002);  // Motor direction register
  ...
  // Your motor controller library might not require additional setup
...
void loop()
{
    // Get motor speed from Modbus register
  int motorSpeed = regBank.get(40001);
  
  // Get motor direction from Modbus register
  int motorDirection = regBank.get(40002);

  // Set motor speed using your library's method
  motorController.setSpeed(abs(motorSpeed)); // Use absolute value to ensure positive speed

  // Set motor direction based on the value from the Modbus register
  if (motorDirection == 1) {
  ...
  // Your other motor-related logic here...
